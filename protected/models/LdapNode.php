...
class LdapNode extends CLdapRecord
{
    ...
    {
        if ($this->hasAttribute("sstExternalDomainName")) {
            return $this->sstExternalDomainName;
        } else {
            return null;
        }
    }
    ...
    {
        // Get the custom external Domain for the spice url
        $externalDomain = $this->getExternalDomain();

        $ip = (Yii::app()->params['virtualization']['spiceByName'] ? ($externalDomain != null) ? $externalDomain : $this->getName() : $this->getVLanIP('pub'));
        if ('0.0.0.0' == $ip) {
