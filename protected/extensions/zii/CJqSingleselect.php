 ...
 * @since 1.1
 */
class CJqSingleselect extends CJuiWidget
{
	public $singleScriptFile=array('jquery.singleselect.js');
	public $singleCssFile='singleselect_core.css';
	...
	public function run()
	{
		Yii::log("CJqSingleselect run");
		Yii::log(print_r($this->values, true));

		echo CHtml::openTag($this->tagName)."\n";
		echo '<select id="' . $this->id . '_singleselect" size="' . $this->size . '"';
		if ($this->multiselect) {
		...
		$script = '$(\'#' . $this->id . '_singleselect\').singleselect(' . $options . ');';
	...
	protected function registerCoreScripts()
	{
		...
		$assets = dirname(__FILE__).'/singleselect_assets';
		$this->scriptUrl = Yii::app()->assetManager->publish($assets, false, -1, true);

		//$this->scriptUrl=Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('ext.zii.dualselect_assets'));

		$cs=Yii::app()->getClientScript();
		if(is_string($this->singleCssFile))
			$cs->registerCssFile($this->scriptUrl.'/css/'.$this->singleCssFile);
		else if(is_array($this->singleCssFile))
		{
			foreach($this->singleCssFile as $cssFile)
				$cs->registerCssFile($this->scriptUrl.'/css/'.$cssFile);
		}

		if(is_string($this->singleScriptFile))
			$this->registerScriptFile('js/'.$this->singleScriptFile);
		else if(is_array($this->singleScriptFile))
		{
			foreach($this->singleScriptFile as $scriptFile)
